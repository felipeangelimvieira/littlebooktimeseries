...
def mean_window_normalizer() -> Callable[[np.ndarray], Tuple[Callable, Callable]]:
    ...
    -------
    strategy : callable
        When called with `lags: np.ndarray`, returns a pair `(transform, inverse)`.
        - `transform(lags, target) -> (lags_n, target_n)` divides by mean(lags).
        - `inverse(y_n) -> y` multiplies by mean(lags) to return to original scale.
    """

    def strategy(lags: np.ndarray):
        lags = np.asarray(lags, dtype=float)
        m = float(np.nanmean(lags)) if lags.size else 1.0
        if not np.isfinite(m) or abs(m) < 1e-12:
            m = 1.0

        def transform(lags_in: np.ndarray, target: Optional[float]):
            lags_arr = np.asarray(lags_in, dtype=float)
            lags_n = lags_arr / m
            tgt_n = None if target is None else float(target) / m
            return lags_n, tgt_n

        def inverse(y_n: float):
            return float(y_n) * m
...
def _iter_series_groups(y: pd.Series):
    ...
    # keep order stable
    for ids, y_g in y.groupby(level=id_lvls, sort=False):
...
def _normalize_supervised_rowwise(
    ...
    for i in range(len(Xt_out)):
        idx = Xt_out.index[i]
        lags = Xt_out.loc[idx, lag_cols].to_numpy(dtype=float)
        transform, _inverse = normalization_strategy(lags)  # per-window
        lags_n, tgt_n = transform(lags, yt_out.loc[idx])
        Xt_out.loc[idx, lag_cols] = lags_n
        yt_out.loc[idx] = float(tgt_n)
...
def _steps_and_full_future_for_group(
    ...
    if req_times is not None:
        ...
        if isinstance(train_time_index, pd.DatetimeIndex):
            freq = train_time_index.freq or _infer_freq_from_index(train_time_index)
            if freq is None:
                freq = pd.infer_freq(train_time_index) or "D"
            rng = pd.date_range(
                start=last_t + pd.tseries.frequencies.to_offset(freq),
                end=max_t,
                freq=freq,
        ...
        elif np.issubdtype(train_time_index.dtype, np.integer):
            H = int(max_t - last_t)
...
class GlobalReductionForecaster(BaseForecaster):
    ...
    *concurrent* exogenous ``X`` at each target timestamp). For each series id,
    ...
    def _predict(
        ...
        # Assemble output
        idx_union = _union_indices(out_index)
        y_pred = pd.Series(
            np.concatenate(out_vals), index=idx_union, name=self._y_name_
        )
    ...
    def _update(
        ...
        for ids, y_flat in _iter_series_groups(y):
            ...
            self._train_time_index_[ids] = pd.Index(
                list(self._train_time_index_[ids]) + list(y_flat.index)
            )
...
def make_reduction(
    ...
    """
    Construct a GlobalReductionForecaster.
    ...
    return GlobalReductionForecaster(
        estimator=estimator,
        window_length=window_length,
        steps_ahead=K,
